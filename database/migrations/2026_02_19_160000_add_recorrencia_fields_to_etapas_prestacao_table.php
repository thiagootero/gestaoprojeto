...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('etapas_prestacao', function (Blueprint $table) {
            $table->string('prestacao_grupo_id', 36)->nullable()->index()->after('projeto_id');
            $table->string('recorrencia_tipo', 20)->nullable()->after('prestacao_grupo_id');
    ...
    {
        Schema::table('etapas_prestacao', function (Blueprint $table) {
            $table->dropIndex(['prestacao_grupo_id']);
            $table->dropColumn([
                'prestacao_grupo_id',
                'recorrencia_tipo',
                'recorrencia_inicio',
